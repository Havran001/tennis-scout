    ...
    btn.addEventListener('click',function(){
      ...
      function nx(i){
        ...
        .then(function(cr){
          ...
          var _noVa2=cd&&cd.matches&&cd.matches.length>0&&(!cd.matches[0].va_pct||!cd.matches[0].dr||(cd.updated&&new Date(cd.updated).getTime()<1774890000000));if(!_noVa2&&cd&&cd.source==='tennisabstract'&&cd.updated&&(Date.now()-new Date(cd.updated))/86400000<7){
          ...
          .then(function(html){
            ...
            var _noVa=cd&&cd.matches&&cd.matches.length>0&&(!cd.matches[0].va_pct||!cd.matches[0].dr||(cd.updated&&new Date(cd.updated).getTime()<1774890000000));if(!ms||ms.length<5||(ms.length<=cn&&!_noVa)){sk++;dn++;prog.innerHTML=window._tsProgress=dn+'/'+tot+' \u2705'+im+' \u23ed'+sk+' \u274c'+er+(dn===tot&&window._importFailed&&window._importFailed.length?'<br><span style="color:#f87171;font-size:10px;line-height:1.8">\u274c Chyby:<br>'+window._importFailed.join('<br>')+'</span>':'');setTimeout(function(){nx(i+1);},200);return;}
