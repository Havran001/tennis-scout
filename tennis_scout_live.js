...
function addWinner(hw){var tries=0;var t=setInterval(function(){var i1=hw.querySelector('#h2h-p1'),i2=hw.querySelector('#h2h-p2');var p1=i1?i1.value.trim():'',p2=i2?i2.value.trim():'';var rows=Array.from(hw.querySelectorAll('#h2h-result tr'));var added=0;rows.forEach(function(row){var tds=row.querySelectorAll('td');if(tds.length<8)return;var wl=tds[7].textContent.trim();if(wl!=='W'&&wl!=='L')return;if(tds[6].querySelector('.h2h-win'))return;if(!p1||!p2)return;var b=document.createElement('b');b.className='h2h-win';b.textContent=' '+(wl==='W'?p1:p2).split(' ').pop();b.style.cssText='color:#FFD700;font-weight:700;';tds[6].appendChild(b);added++;});tries++;if(added>0||tries>10)clearInterval(t);},300);}
setInterval(function(){var sh=(document.getElementById('ts-host')||{}).shadowRoot;if(!sh)return;var hw=sh.getElementById('h2hw');if(!hw||hw._acp)return;if(typeof hw.render!=='function')return;hw._acp=true;var or=hw.render;hw.render=function(){or.call(hw);addAC(hw);addWinner(hw);};},300);
})();