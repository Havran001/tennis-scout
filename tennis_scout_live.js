    ...
    if(!shown.length){h+='<div style="padding:60px;text-align:center;color:rgba(255,255,255,.2);">Žádné zápasy</div>';}
      ...
      if(activeSort==='time'){
        ...
        shown.forEach(function(m){
          ...
          h+='<div class="mrow" data-url="'+m.url+'" style="border-left:3px solid '+(isLive?'#ff1100':'transparent')+';background:'+(isLive?'rgba(255,17,0,.025)':'transparent')+';padding:7px 0 7px 10px;cursor:pointer;transition:background .1s;">';
...
function buildUI(){
  ...
  function goView(view){
    ...
    if(view==='matches'){sh.querySelectorAll('.nav-item').forEach(n=>n.classList.remove('active'));sh.getElementById('nav-matches')?.classList.add('active');sh.querySelectorAll('.mg').forEach(m=>m.style.display='none');['pw','home-view','filterbar','mnav'].forEach(id=>{var e=sh.getElementById(id);if(e)e.style.display='none';});var mwx=sh.getElementById('mw');if(mwx){mwx.style.display='block';if(mwx.render)mwx.render();}return;}
          if(view==='matches_backup'){sh.querySelectorAll('.nav-item').forEach(n=>n.classList.remove('active'));sh.getElementById('nav-matches-backup')?.classList.add('active');sh.querySelectorAll('.mg').forEach(m=>m.style.display='none');['pw','home-view','filterbar','mnav'].forEach(id=>{var e=sh.getElementById(id);if(e)e.style.display='none';});var mwx=sh.getElementById('mw-backup');if(mwx){mwx.style.display='block';if(mwx.render)mwx.render();}return;}
    // Update sidebar
...
// === ZÁLOHA MODULU ZÁPASY ===
function renderMatchesBackupBackup(data){
    var all=getMatches(data);
    var mcEl=sh.getElementById('nav-matches-count');
    if(mcEl)mcEl.textContent=all.length;
    var hcm=sh.getElementById('hc-count-m');if(hcm)hcm.textContent=all.length+' zápasů';
    ...
    var fin=all.filter(function(m){return m.isFin;});
    var sch=all.filter(function(m){return m.isSch;});
    var shown=activeFilterBak==='live'?live:activeFilterBak==='finished'?fin:activeFilterBak==='scheduled'?sch:all;
    // Filter by tier
    if(activeTierBak!=='all'){shown=shown.filter(function(m){var ti=tInfo(m.tournament||'');var l=ti.l||'';if(activeTierBak==='GS')return l==='Grand Slam';if(activeTierBak==='M1000')return l==='ATP 1000'||l==='WTA 1000';if(activeTierBak==='ATP500')return l==='ATP 500'||l==='WTA 500';if(activeTierBak==='ATP250')return l==='ATP 250'||l==='WTA 250';if(activeTierBak==='WTA')return l.startsWith('WTA');if(activeTierBak==='CH')return l==='Challenger';if(activeTierBak==='ITF')return l==='ITF';return true;});}
    // Filter by format (singles/doubles)
    if(activeFormatBak==='singles')shown=shown.filter(function(m){return !(m.tournament||'').includes('DOUBLES');});
    if(activeFormatBak==='doubles')shown=shown.filter(function(m){return (m.tournament||'').includes('DOUBLES');});
    if(activeTierBak!=='all'){shown=shown.filter(function(m){var ti=tInfo(m.tournament||'');var l=ti.l||'';if(activeTierBak==='GS')return l==='Grand Slam';if(activeTierBak==='M1000')return l==='ATP 1000'||l==='WTA 1000';if(activeTierBak==='ATP500')return l==='ATP 500'||l==='WTA 500';if(activeTierBak==='ATP250')return l==='ATP 250'||l==='WTA 250';if(activeTierBak==='WTA')return l.startsWith('WTA');if(activeTierBak==='CH')return l==='Challenger';if(activeTierBak==='ITF')return l==='ITF';return true;});}
    if(activeFormatBak==='singles')shown=shown.filter(function(m){return !(m.tournament||'').toUpperCase().includes('DOUBLES');});
    if(activeFormatBak==='doubles')shown=shown.filter(function(m){return (m.tournament||'').toUpperCase().includes('DOUBLES');});
    if(activeSortBak==='time')shown=shown.slice().sort(function(a,b){return (a.ts||0)-(b.ts||0);});

    var h='<div style="padding:0 20px 60px;">';
    h+='<div style="display:flex;align-items:center;gap:6px;padding:12px 0 10px;border-bottom:1px solid rgba(255,255,255,.06);">';
    [{d:-1,l:'Včera'},{d:0,l:'Dnes'},{d:1,l:'Zítra'},{d:2,l:'+2 dny'},{d:3,l:'+3 dny'}].forEach(function(x){
      var on=activeDayBak.indexOf(x.d)>=0;
      h+='<button data-day="'+x.d+'" style="padding:5px 16px;border-radius:7px;border:1px solid '+(on?'#00C853':'rgba(255,255,255,.1)')+';background:'+(on?'rgba(0,200,83,.15)':'transparent')+';color:'+(on?'#00C853':'rgba(255,255,255,.4)')+';font-size:12px;cursor:pointer;font-weight:'+(on?700:400)+';">'+x.l+'</button>';
    });
    var _fo=(activeDayBak.indexOf('fav')>=0);
    h+='<button data-day="fav" style="padding:5px 16px;border-radius:7px;border:1px solid '+(_fo?'#FFD700':'rgba(255,255,255,.1)')+';background:'+(_fo?'rgba(255,215,0,.12)':'transparent')+';color:'+(_fo?'#FFD700':'rgba(255,255,255,.4)')+';font-size:12px;cursor:pointer;margin-left:4px;font-weight:'+(_fo?700:400)+';">&#9733; Obl&#237;ben&#233;'+(JSON.parse(localStorage.getItem('ts_favs')||'[]').length?' <span style="font-size:10px;background:rgba(255,215,0,.25);border-radius:8px;padding:1px 6px;">'+JSON.parse(localStorage.getItem('ts_favs')||'[]').length+'</span>':'')+'</button>';
    h+='<div style="margin-left:auto;display:flex;align-items:center;gap:6px;">';
    if(data.src==='github'){var _upd=data.updated?new Date(data.updated).getTime():NaN;var age=isNaN(_upd)?null:Math.round((Date.now()-_upd)/60);if(age!==null&&age>60)h+='<span style="font-size:9px;color:rgba(255,140,0,.7);">⚠️ data '+age+'min stará — spusť skript na flashscore.com</span>';}
    if(_lastUpdatedBak)h+='<span style="font-size:9px;color:rgba(255,255,255,.2);">♥ '+_lastUpdatedBak.slice(11,16)+'</span>';
    h+='<span style="width:6px;height:6px;background:#00C853;border-radius:50%;display:inline-block;"></span>';
    h+='</div></div>';
    h+='<div style="display:flex;gap:4px;padding:8px 0 6px;">';
    [['all','Vše',all.length],['live','LIVE 🔴',live.length],['finished','Konec',fin.length],['scheduled','Náplán.',sch.length]].forEach(function(f){
      var on=activeFilterBak===f[0];
      h+='<button data-filter="'+f[0]+'" style="padding:3px 9px;border-radius:10px;border:1px solid '+(on?'#00C853':'rgba(255,255,255,.08)')+';background:'+(on?'rgba(0,200,83,.15)':'transparent')+';color:'+(on?'#00C853':'rgba(255,255,255,.35)')+';font-size:9px;cursor:pointer;font-weight:'+(on?700:400)+';">'+f[1]+' <span style="opacity:.6;">'+f[2]+'</span></button>';
    });
    h+='<div style="display:flex;gap:4px;padding:4px 0 0;flex-wrap:wrap;align-items:center;">';
    [['all','Vše'],['GS','Grand Slam'],['M1000','ATP 1000'],['ATP500','ATP 500'],['ATP250','ATP 250'],['WTA','WTA'],['CH','Challenger'],['ITF','ITF']].forEach(function(t){var on=activeTierBak===t[0];h+='<button data-tier="'+t[0]+'" style="padding:2px 9px;border-radius:7px;border:1px solid '+(on?'#00C853':'rgba(255,255,255,.08)')+';background:'+(on?'rgba(0,200,83,.15)':'transparent')+';color:'+(on?'#00C853':'rgba(255,255,255,.3)')+';font-size:9px;font-weight:'+(on?700:400)+';cursor:pointer;">'+t[1]+'</button>';});
    h+='<div style="width:1px;height:14px;background:rgba(255,255,255,.1);margin:0 3px;flex-shrink:0;align-self:center;"></div>';
    [['all','Vše'],['singles','Dvouhra'],['doubles','Čtyřhra']].forEach(function(f){var on=activeFormatBak===f[0];h+='<button data-fmt="'+f[0]+'" style="padding:2px 9px;border-radius:7px;border:1px solid '+(on?'#00C853':'rgba(255,255,255,.08)')+';background:'+(on?'rgba(0,200,83,.15)':'transparent')+';color:'+(on?'#00C853':'rgba(255,255,255,.3)')+';font-size:9px;font-weight:'+(on?700:400)+';cursor:pointer;">'+f[1]+'</button>';});
    h+='</div>';
    h+='<div style="margin-left:auto;"><button data-sort="1" style="padding:3px 10px;border-radius:10px;border:1px solid '+(activeSortBak==='time'?'#00C853':'rgba(255,255,255,.15)')+';background:'+(activeSortBak==='time'?'rgba(0,200,83,.15)':'transparent')+';color:'+(activeSortBak==='time'?'#00C853':'rgba(255,255,255,.4)')+';font-size:9px;cursor:pointer;">⏱ Čas</button> <button data-sort="0" style="padding:3px 10px;border-radius:10px;border:1px solid '+(activeSortBak==='tournament'?'#00C853':'rgba(255,255,255,.15)')+';background:'+(activeSortBak==='tournament'?'rgba(0,200,83,.15)':'transparent')+';color:'+(activeSortBak==='tournament'?'#00C853':'rgba(255,255,255,.4)')+';font-size:9px;cursor:pointer;">🏆 Turnaj</button></div>';
    h+='</div>';
    if(!shown.length){h+='<div style="padding:60px;text-align:center;color:rgba(255,255,255,.2);">Žádné zápasy</div>';}
    else{
      if(activeSortBak==='time'){
        // Flat list sorted by time
        shown.forEach(function(m){
          var isLive=m.isLive;var ns=Math.max((m.sets1||[]).length,(m.sets2||[]).length);var _s1=(m.sets1||[]).reduce(function(a,v,i){return a+(parseInt(v)>parseInt((m.sets2||[])[i]||0)?1:0);},0),_s2=(m.sets2||[]).reduce(function(a,v,i){return a+(parseInt(v)>parseInt((m.sets1||[])[i]||0)?1:0);},0);var _hasSets=_s1>0||_s2>0;var w1=m.isFin&&(_hasSets?_s1>_s2:m.winner===1),w2=m.isFin&&(_hasSets?_s2>_s1:m.winner===2);
          var ti=tInfo(m.tournament||'');
...
var _f=JSON.parse(localStorage.getItem('ts_favs')||'[]');if(_f.length){wrap.querySelectorAll('button').forEach(function(_b){var _m=_b.getAttribute('onclick');if(!_m)return;var _i=_m.indexOf("id='")+4;var _j=_m.indexOf("'",_i);var _id=_m.substring(_i,_j);if(_f.indexOf(_id)>-1){_b.style.color='#FFD700';var _svg=_b.querySelector('svg');if(_svg)_svg.style.fill='currentColor';}});}
    wrap.querySelectorAll('[data-day]').forEach(function(btn){btn.addEventListener('click',function(){var _dv=btn.dataset.day,_d=(_dv==='fav'?'fav':parseInt(_dv));var _i=activeDayBak.indexOf(_d);if(_i>=0){if(activeDayBak.length>1)activeDayBak.splice(_i,1);}else{if(_dv==='fav'){activeDayBak=['fav'];}else{var _fi=activeDayBak.indexOf('fav');if(_fi>=0)activeDayBak.splice(_fi,1);activeDayBak.push(_d);}}render();});});
    wrap.querySelectorAll('[data-tier]').forEach(function(btn){btn.addEventListener('click',function(){activeTierBak=btn.dataset.tier;if(_lastData)renderMatchesBackup(_lastData);});});
    wrap.querySelectorAll('[data-fmt]').forEach(function(btn){btn.addEventListener('click',function(){activeFormatBak=btn.dataset.fmt;if(_lastData)renderMatchesBackup(_lastData);});});
    wrap.querySelectorAll('[data-sort]').forEach(function(btn){btn.addEventListener('click',function(){activeSortBak=btn.dataset.sort==='1'?'time':'tournament';if(_lastData)renderMatchesBackup(_lastData);});});
    wrap.querySelectorAll('[data-tier]').forEach(function(btn){btn.addEventListener('click',function(){activeTierBak=btn.dataset.tier;if(_lastData)renderMatchesBackup(_lastData);});});
    wrap.querySelectorAll('[data-fmt]').forEach(function(btn){btn.addEventListener('click',function(){activeFormatBak=btn.dataset.fmt;if(_lastData)renderMatchesBackup(_lastData);});});
    wrap.querySelectorAll('[data-filter]').forEach(function(btn){btn.addEventListener('click',function(){activeFilterBak=btn.dataset.filter;if(_lastData)renderMatchesBackup(_lastData);});});
    wrap.querySelectorAll('.mrow').forEach(function(row){
      row.addEventListener('mouseover',function(){row.style.background='rgba(255,255,255,.04)';});
      row.addEventListener('mouseout',function(){row.style.background='transparent';});
      row.addEventListener('click',function(e){if(e.target.closest('.mc-plink'))return;if(row.dataset.url)window.open(row.dataset.url,'_blank');});
  ...
  async function tick(){
    ...
    _fetching=true;
    try{
      var data=await loadDataBackup();
      _lastUpdatedBak=data.updated||new Date().toISOString();
      _lastData=data;
      renderMatchesBackup(data);
    }catch(e){
      if(_lastData){renderMatchesBackup(_lastData);}
      else{wrap.innerHTML='<div style="padding:60px;text-align:center;color:rgba(255,255,255,.2);">⚠️ '+e.message+'</div>';}
    }finally{_fetching=false;}
  }
  function render(){if(!_lastData)wrap.innerHTML='<div style="padding:60px;text-align:center;color:rgba(255,255,255,.2);">⏳ Načítám...</div>';tick();}
  wrap.render=function(){if(wrap.style.display==='none')return;if(_intervalBak)clearInterval(_intervalBak);render();_intervalBak=setInterval(tick,10000);};
  wrap.destroy=function(){if(_intervalBak){clearInterval(_intervalBak);_intervalBak=null;}};
  return wrap;
}
