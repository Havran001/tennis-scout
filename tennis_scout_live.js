...
setInterval(function(){var sh=(document.getElementById('ts-host')||{}).shadowRoot;if(!sh)return;var cells=Array.from(sh.querySelectorAll('td.ta-odds'));if(!cells.length)return;var cache=window.__phCache||{};var circleStyle='font-family:monospace;font-size:10px;font-weight:700;color:#FFD700;border:1.5px solid #FFD700;border-radius:50%;width:36px;height:18px;display:inline-flex;align-items:center;justify-content:center;';var plainStyle='font-family:monospace;font-size:10px;font-weight:600;color:rgba(255,255,255,0.5);';var dashStyle='font-family:monospace;font-size:10px;color:rgba(255,255,255,0.3);';cells.forEach(function(td){if(td.dataset.oddsSet)return;var row=td.closest('[data-mid]');if(!row||!row.dataset.mid)return;var parts=row.dataset.mid.split('_');var pid=parts[0],date=parts[1],oppPrefix=parts.slice(2).join('_');if(!cache[pid]){cache[pid]='loading';fetch('https://raw.githubusercontent.com/Havran001/tennis-scout/main/player_history/'+pid+'.json').then(function(r){return r.json();}).then(function(d){cache[pid]=d.matches||[];}).catch(function(){cache[pid]=[];});return;}if(cache[pid]==='loading')return;var matches=cache[pid];var m=matches.find(function(m){if(!m.odds_opp)return false;if(m.date!==date)return false;var oppKey=(m.opponent||'').replace(/\s/g,'').slice(0,oppPrefix.length);return oppKey===oppPrefix;});if(m){var isW=m.result==='W';var hasAlc=!!m.odds_alc;var alcOdds=hasAlc?m.odds_alc.toFixed(2):null;var oppOdds=m.odds_opp.toFixed(2);var alcSpan=hasAlc?(isW?'<span style="'+circleStyle+'">'+alcOdds+'</span>':'<span style="'+plainStyle+'">'+alcOdds+'</span>'):'<span style="'+dashStyle+'">—</span>';var oppSpan=isW?' / <span style="'+plainStyle+'">'+oppOdds+'</span>':' / <span style="'+circleStyle+'">'+oppOdds+'</span>';td.innerHTML=alcSpan+oppSpan;td.dataset.oddsSet='1';}});window.__phCache=cache;},800);