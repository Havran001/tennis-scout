...
function buildH2HTab(sh){
  ...
  function renderH2H(p1,p2,hist1,hist2){
    ...
    if(h2h.length){
      ...
      html+='<thead><tr>';
      ['Datum','Turnaj','Povrch','Kolo','Skóre'].forEach(function(c){html+='<th style="padding:5px 8px;font-size:8px;color:rgba(255,255,255,.25);letter-spacing:1px;text-align:left;border-bottom:1px solid rgba(255,255,255,.06);">'+c+'</th>';});
      html+='</tr></thead><tbody>';
...
(function waitForH2H(){
  ...
  var timer=setInterval(function(){
    ...
    function renderH2H(p1,p2,h1,h2){
      ...
      if(h2hM.length){html+='<div style="margin-top:14px;"><div style="font-size:9px;color:rgba(255,255,255,.25);letter-spacing:2px;text-transform:uppercase;margin-bottom:8px;padding-bottom:8px;border-bottom:1px solid rgba(255,255,255,.04);">Vzájemné zápasy ('+h2hM.length+')</div><table style="width:100%;border-collapse:collapse;"><thead><tr>'+['Datum','Turnaj','Povrch','Kolo','Skóre'].map(function(c){return '<th style="padding:4px 8px;font-size:8px;color:rgba(255,255,255,.25);letter-spacing:1px;text-align:left;border-bottom:1px solid rgba(255,255,255,.06);">'+c+'</th>';}).join('')+'</tr></thead><tbody>'+h2hM.map(function(m,i){var bg=i%2===0?'rgba(255,255,255,0)':'rgba(255,255,255,.025)';var surf=(m.surface||'').toLowerCase();var sl=surf.includes('clay')?'Antuka':surf.includes('grass')?'Tráva':'Tvrdý';var sc=surf.includes('clay')?'#fb923c':surf.includes('grass')?'#4ade80':'#60a5fa';var ds=m.date?(m.date.slice(8,10)+'.'+m.date.slice(5,7)+'.'+m.date.slice(0,4)):'';var td='padding:5px 8px;font-size:12px;white-space:nowrap;';var wl=m.result||'';return '<tr style="background:'+bg+'" data-wl="'+wl+'" data-score="'+(m.score||'')+'">'+'<td style="'+td+'color:rgba(255,255,255,.4);">'+ds+'</td>'+'<td style="'+td+'color:rgba(255,255,255,.7);">'+( m.tournament||'—' )+'</td>'+'<td><span style="font-size:9px;font-weight:700;padding:2px 6px;border-radius:3px;background:rgba(255,255,255,.05);color:'+sc+';">'+sl+'</span></td>'+'<td style="'+td+'color:rgba(255,255,255,.5);">'+( m.round||'—' )+'</td>'+'<td style="'+td+'font-weight:700;font-family:monospace;color:#e6edf3;">'+( m.score||'—' )+'</td>'+'</tr>';}).join('')).join('')+'</tbody></table></div>';}
...
(function(){
function addAC(hw){setTimeout(function(){['h2h-p1','h2h-p2'].forEach(function(id){var i=hw.querySelector('#'+id);if(!i)return;var old=i.parentNode.querySelector('.ts-ac-dd');if(old)old.remove();var dd=document.createElement('div');dd.className='ts-ac-dd';dd.style.cssText='position:absolute;top:100%;left:0;right:0;background:#1c2128;border:1px solid rgba(255,255,255,.15);border-radius:8px;z-index:9999;max-height:220px;overflow-y:auto;margin-top:3px;box-shadow:0 8px 24px rgba(0,0,0,.6);display:none;';i.parentNode.style.position='relative';i.parentNode.appendChild(dd);i.oninput=function(){var q=i.value.trim().toLowerCase();dd.innerHTML='';if(!q){dd.style.display='none';return;}var ms=(window.ATP_PLAYERS||[]).filter(function(p){return(p.name||'').toLowerCase().includes(q)||(p.full_name||'').toLowerCase().includes(q);}).slice(0,10);if(!ms.length){dd.style.display='none';return;}ms.forEach(function(p){var it=document.createElement('div');it.style.cssText='padding:8px 12px;cursor:pointer;font-size:13px;color:#e6edf3;border-bottom:1px solid rgba(255,255,255,.05);display:flex;align-items:center;justify-content:space-between;';it.innerHTML='<span style="font-weight:600;">'+(p.full_name||p.name)+'</span><span style="font-size:10px;color:rgba(255,255,255,.35);">#'+p.rank+'</span>';it.onmousedown=function(e){e.preventDefault();i.value=p.full_name||p.name;dd.style.display='none';};it.onmouseover=function(){it.style.background='rgba(0,200,83,.1)';};it.onmouseout=function(){it.style.background='';};dd.appendChild(it);});dd.style.display='block';};i.onblur=function(){setTimeout(function(){dd.style.display='none';},150);};});},0);}
setInterval(function(){var sh=(document.getElementById('ts-host')||{}).shadowRoot;if(!sh)return;var hw=sh.getElementById('h2hw');if(!hw||hw._acp)return;if(typeof hw.render!=='function')return;hw._acp=true;var or=hw.render;hw.render=function(){or.call(hw);addAC(hw);};},300);
setInterval(function(){var sh=(document.getElementById('ts-host')||{}).shadowRoot;if(!sh)return;var hw=sh.getElementById('h2hw');if(!hw)return;var i1=hw.querySelector('#h2h-p1'),i2=hw.querySelector('#h2h-p2');var p1=i1?i1.value.trim():'',p2=i2?i2.value.trim():'';if(!p1||!p2)return;Array.from(hw.querySelectorAll('#h2h-result tr[data-wl]')).forEach(function(row){var wl=row.dataset.wl;if(wl!=='W'&&wl!=='L')return;var tds=row.querySelectorAll('td');if(tds.length<5)return;var sc=tds[4];if(sc.querySelector('.h2h-win'))return;var b=document.createElement('b');b.className='h2h-win';b.textContent=' '+(wl==='W'?p1:p2).split(' ').pop();b.style.cssText='color:#FFD700;font-weight:700;';sc.appendChild(b);});},500);
})();