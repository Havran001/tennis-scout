...
(function waitForH2H(){
  ...
  var timer=setInterval(function(){
    ...
    function renderH2H(p1,p2,h1,h2){
      ...
      if(h2hM.length){html+='<div style="margin-top:14px;"><div style="font-size:9px;color:rgba(255,255,255,.25);letter-spacing:2px;text-transform:uppercase;margin-bottom:8px;padding-bottom:8px;border-bottom:1px solid rgba(255,255,255,.04);">Vzájemné zápasy ('+h2hM.length+')</div><table style="width:100%;border-collapse:collapse;"><thead><tr>'+['Datum','Turnaj','Povrch','Kolo','Skóre'].map(function(c){return '<th style="padding:4px 8px;font-size:8px;color:rgba(255,255,255,.25);letter-spacing:1px;text-align:left;border-bottom:1px solid rgba(255,255,255,.06);">'+c+'</th>';}).join('')+'</tr></thead><tbody>'+h2hM.map(function(m,i){var wn=m.result==='W'?p1n:p2n;var ws=m.result&&m.score?' <b class="h2h-win" style="color:#FFD700;font-weight:700;">'+wn.split(' ').pop()+'</b>':'';var bg=i%2===0?'rgba(255,255,255,0)':'rgba(255,255,255,.025)';var surf=(m.surface||'').toLowerCase();var sl=surf.includes('clay')?'Antuka':surf.includes('grass')?'Tráva':'Tvrdý';var sc=surf.includes('clay')?'#fb923c':surf.includes('grass')?'#4ade80':'#60a5fa';var td='padding:5px 8px;font-size:12px;color:rgba(255,255,255,.7);white-space:nowrap;';return '<tr style="background:'+bg+'">'+'<td style="'+td+'color:rgba(255,255,255,.4);">'+( m.date?(m.date.slice(8,10)+'.'+m.date.slice(5,7)+'.'+m.date.slice(0,4)):'' )+'</td>'+'<td style="'+td+'">'+( m.tournament||'—' )+'</td>'+'<td><span style="font-size:9px;font-weight:700;padding:2px 6px;border-radius:3px;background:rgba(255,255,255,.05);color:'+sc+';">'+sl+'</span></td>'+'<td style="'+td+'">'+( m.round||'—' )+'</td>'+'<td style="'+td+'font-weight:700;font-family:monospace;">'+( m.score||'—' )+ws+'</td>'+'</tr>';}).join('')).join('')+'</tbody></table></div>';}
