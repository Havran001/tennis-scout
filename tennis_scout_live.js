    ...
    if(!shown.length){h+='<div style="padding:60px;text-align:center;color:rgba(255,255,255,.2);">Žádné zápasy</div>';}
      ...
      if(activeSort==='time'){
        ...
        shown.forEach(function(m){
          ...
          h+='</div>';
          // Score
          var scoreH='';if(ns>0){var setsStr='';var _sc1=0,_sc2=0;for(var si=0;si<ns;si++){var sv1=parseInt(m.sets1[si]||0),sv2=parseInt(m.sets2[si]||0);if(sv1>sv2)_sc1++;else if(sv2>sv1)_sc2++;if(setsStr)setsStr+=' ';setsStr+=sv1+':'+sv2;}if(m.isFin&&(_sc1>0||_sc2>0)){h+='<div style="font-size:13px;font-weight:700;color:#fff;letter-spacing:0.5px;line-height:1.2;margin-left:8px;">'+_sc1+':'+_sc2+'</div>';}}
