...
function renderMatches(data){
    ...
    [['all','VÅ¡e',all.length],['live','LIVE ð´',live.length],['finished','Konec',fin.length],['scheduled','NÃ¡plÃ¡n.',sch.length],['odds','S kurzem ð°',withOdds.length]].forEach(function(f){
    ...
    if(!shown.length){h+='<div style="padding:60px;text-align:center;color:rgba(255,255,255,.2);">Å½Ã¡dnÃ© zÃ¡pasy</div>';}
      ...
      if(activeSort==='time'){
        ...
        shown.forEach(function(m){
          ...
          h+='<div class="mrow" data-url="'+m.url+'" style="border-left:3px solid '+(isLive?'#ff1100':'transparent')+';background:'+(isLive?'rgba(255,17,0,.025)':'transparent')+';position:relative;padding:7px 0 7px 10px;cursor:pointer;transition:background .1s;">';
...
function buildUI(){
  ...
  sidebar.innerHTML=`
    ...
    <nav id="sb-nav">
      ...
      <div class="nav-item" data-view="matches" id="nav-matches"><span class="nav-icon">ð¾</span> ZÃ¡pasy</div><div class="nav-item" data-view="matches_backup" id="nav-matches-backup"><span class="nav-icon">ð</span> ZÃ¡p. zÃ¡loha
    ...
    <div id="sb-footer">
      ...
      <button id="sb-fs-import">â¬ Import Flashscore (rok)</button>
    </div>
  `;
  ...
  // FLASHSCORE IMPORT
  (function(){
    var btn=sh.getElementById('sb-fs-import');
    if(!btn)return;
    btn.addEventListener('click',function(){
      var self=this,prog=sh.getElementById('fs-progress');
      var GH=localStorage.getItem('ts_gh_token');
      if(!GH){GH=prompt('Zadej GitHub token:');if(!GH)return;localStorage.setItem('ts_gh_token',GH);}
      self.disabled=true;prog.style.display='block';
      prog.innerHTML='NaÄÃ­tÃ¡m zÃ¡pasy z Flashscore...';

      var PROXY='https://tennis-proxy.vavra-radovan.workers.dev/';
      var START=-365,END=-2;
      var pidCache={},histCache={};
      var added=0,days=0,errs=0;

      // Name â ATP ID lookup
      var nameToId={};
      var lastToPlayers={};
      (window.ATP_PLAYERS||[]).forEach(function(p){
        var pid=p.id,name=p.name||'',full=p.full_name||'';
        if(!pid)return;
        var parts=name.split(' ');
        if(parts.length>=2){var fs=parts.slice(1).join(' ')+' '+parts[0];nameToId[fs.toLowerCase()]=pid;}
        nameToId[name.toLowerCase()]=pid;
        if(full)nameToId[full.toLowerCase()]=pid;
        var last=(parts[parts.length-1]||'').toLowerCase();
        if(!lastToPlayers[last])lastToPlayers[last]=[];
        lastToPlayers[last].push({id:pid,name:name});
      });

      function findPid(raw){
        if(!raw)return null;
        var nl=raw.trim().toLowerCase();
        if(nameToId[nl])return nameToId[nl];
        var parts=nl.split(' ');
        var last=parts[0]||'';
        var initial=parts[1]?parts[1][0]:'';
        var cands=lastToPlayers[last]||[];
        if(cands.length===1)return cands[0].id;
        for(var c of cands){var cp=c.name.split(' ');if(initial&&cp[0]&&cp[0][0].toLowerCase()===initial)return c.id;}
        return null;
      }

      function loadHist(pid){
        if(histCache[pid])return Promise.resolve(histCache[pid]);
        return fetch('https://raw.githubusercontent.com/Havran001/tennis-scout/main/player_history/'+pid+'.json?v='+Date.now())
          .then(function(r){return r.ok?r.json():null;})
          .then(function(d){histCache[pid]=d||{matches:[]};return histCache[pid];})
          .catch(function(){histCache[pid]={matches:[]};return histCache[pid];});
      }

      function processDay(day){
        return fetch(PROXY+'?day='+day+'&t='+Date.now())
          .then(function(r){return r.ok?r.json():null;})
          .then(function(data){
            if(!data)return;
            var matches=data.matches||(data.days?Object.values(data.days).flat():[]);
            var today=new Date();
            matches.forEach(function(m){
              ...
              var d2=new Date(today);d2.setDate(d2.getDate()+day);
              var ds=d2.toISOString().slice(0,10).replace(/-/g,'');
              var s1w=sets1.filter(function(a,i){return String(a)>String(sets2[i]);}).length;
              var s2w=sets2.filter(function(a,i){return String(a)>String(sets1[i]);}).length;

              [[p1,p2,winner===1,s1w,s2w],[p2,p1,winner===2,s2w,s1w]].forEach(function(row){
                var pn=row[0],opp=row[1],won=row[2],ms=row[3],os=row[4];
                if(pidCache[pn]===undefined)pidCache[pn]=findPid(pn);
                var pid=pidCache[pn];
                if(!pid)return;
                if(!histCache[pid])histCache[pid]={matches:[]};
                var existing=histCache[pid].matches||[];
                if(mid&&existing.find(function(e){return e.id===mid;}))return;
                existing.push({id:mid,date:ds,tournament:tourn,surface:surface,level:'A',round:m.round||'',result:won?'W':'L',opponent:opp,score:ms+'-'+os,best_of:'3',rank:'',opp_rank:'',src:'flashscore'});
                histCache[pid].matches=existing;
                added++;
              });
            });
            days++;
      ...
      // SekvenÄnÄ prochÃ¡zej dny
      var currentDay=START;
      function nextDay(){
        if(currentDay>END){
          // UloÅ¾ vÅ¡e na GitHub
          prog.innerHTML='UklÃ¡dÃ¡m '+Object.keys(histCache).length+' hrÃ¡ÄÅ¯ na GitHub...';
          var pids=Object.keys(histCache);
          var saved=0,saveErr=0;
          var today=new Date().toISOString().slice(0,10);

          function saveNext(i){
            if(i>=pids.length){
              self.disabled=false;
              prog.innerHTML='â Hotovo! '+added+' zÃ¡pasÅ¯ pÅidÃ¡no za '+days+' dnÃ­. UloÅ¾eno: '+saved+' hrÃ¡ÄÅ¯.';
              return;
            }
            var pid=pids[i];
            var hist=histCache[pid];
            if(!hist.matches||hist.matches.length===0){saveNext(i+1);return;}
            hist.matches.sort(function(a,b){return b.date.localeCompare(a.date);});
            hist.total=hist.matches.length;
            hist.updated=today;
            var content=btoa(unescape(encodeURIComponent(JSON.stringify(hist))));
            // ZÃ­skej SHA
            fetch('https://api.github.com/repos/Havran001/tennis-scout/contents/player_history/'+pid+'.json',{
              headers:{'Authorization':'token '+GH,'Accept':'application/vnd.github.v3+json'}
            }).then(function(r){return r.ok?r.json():null;})
            .then(function(gd){
              var body={message:'FS: '+pid,content:content};
              if(gd&&gd.sha)body.sha=gd.sha;
              ...
              if(r&&r.ok)saved++;else saveErr++;
              prog.innerHTML='UklÃ¡dÃ¡m... '+saved+'/'+(i+1)+' hrÃ¡ÄÅ¯';
              setTimeout(function(){saveNext(i+1);},300);
            }).catch(function(){saveErr++;setTimeout(function(){saveNext(i+1);},300);});
          }
          saveNext(0);
          return;
        }
        prog.innerHTML='Den '+currentDay+'/'+END+' | pÅidÃ¡no: '+added;
        processDay(currentDay).then(function(){
          currentDay++;
          setTimeout(nextDay,100);
        });
      }
      nextDay();
    });
...
var _kbScrapeUrl=_kbUrl.replace('/kb-odds','/kb-scrape');
var _runKb=function(){
  // NejdÅÃ­v vynutÃ­ scrape, pak naÄti ÄerstvÃ¡ data
...
var _runFortuna=function(){
  // Volej oba scrape endpointy paralelnÄ
