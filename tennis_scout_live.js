...
(function addSidebarBtn(){function getSR(){var divs=document.querySelectorAll("div");for(var i=0;i<divs.length;i++){if(divs[i].shadowRoot)return divs[i].shadowRoot;}return null;}var try_=function(){var sr=getSR()||document;if(sr.getElementById("bulk-odds-btn"))return;var btns=sr.querySelectorAll("button");var imp=null;for(var i=0;i<btns.length;i++){if(btns[i].textContent.trim().indexOf("Tennis Abstract")>=0){imp=btns[i];break;}}if(!imp)return;var b=document.createElement("button");b.id="bulk-odds-btn";b.innerHTML="\u26a1 Bulk Odds";b.style.cssText="width:100%;padding:8px 12px;border-radius:6px;cursor:pointer;background:rgba(255,165,0,0.15);border:1px solid rgba(255,165,0,0.4);color:#ffa500;font-size:13px;margin-top:6px;";b.addEventListener("click",function(){bulkOddsImport();});imp.parentElement.insertBefore(b,imp.nextSibling);};[300,800,2000,4000].forEach(function(d){setTimeout(try_,d);});// Also watch for DOM changes
new MutationObserver(function(){try_();}).observe(document.body,{childList:true,subtree:true});})();
// Also add Odds btn to player card via MutationObserver
(function addPlayerOddsBtn(){function getSR(){var divs=document.querySelectorAll("div");for(var i=0;i<divs.length;i++){if(divs[i].shadowRoot)return divs[i].shadowRoot;}return null;}function tryAdd(){var sr=getSR();if(!sr)return;var updateBtn=sr.getElementById("pp-update-btn");if(!updateBtn||sr.getElementById("pp-odds-btn"))return;var pid=updateBtn.getAttribute("data-pid");var fullname=updateBtn.getAttribute("data-fullname");var ob=document.createElement("button");ob.id="pp-odds-btn";ob.setAttribute("data-pid",pid);ob.setAttribute("data-fullname",fullname);ob.innerHTML="\u26a1 Odds";ob.style.cssText="background:rgba(255,165,0,0.15);border:1px solid rgba(255,165,0,0.4);color:#ffa500;border-radius:6px;padding:4px 10px;cursor:pointer;margin-left:6px;font-size:13px;";ob.addEventListener("click",function(){openOddsOnBE(pid,fullname);});updateBtn.parentNode.insertBefore(ob,updateBtn.nextSibling);}new MutationObserver(function(){tryAdd();}).observe(document.body,{childList:true,subtree:true});setTimeout(tryAdd,500);setTimeout(tryAdd,1500);setTimeout(tryAdd,3000);})();
// === MERGE ODDS ON SAVE ===
