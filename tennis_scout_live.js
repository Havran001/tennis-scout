...
function buildMatchesTab(sh){
  ...
  function tInfo(t){
    var u=t.toUpperCase();
    if(u.includes('ITF')) return{i:'ðµ',l:'ITF',c:'rgba(255,255,255,.4)',bg:'rgba(255,255,255,.03)',b:'rgba(255,255,255,.1)'};
    if(u.includes('CHALLENGER')) return{i:'â¡',l:'Challenger',c:'#60a5fa',bg:'rgba(96,165,250,.1)',b:'rgba(96,165,250,.2)'};
    ...
    return{i:'ð¾',l:'ATP 250',c:'rgba(255,255,255,.55)',bg:'rgba(255,255,255,.05)',b:'rgba(255,255,255,.12)'};
  }
...
function renderMatches(data){
    ...
    [{d:-1,l:'VÄera'},{d:0,l:'Dnes'},{d:1,l:'ZÃ­tra'}].forEach(function(x){
    ...
    [['all','VÅ¡e',all.length],['live','LIVE ð´',live.length],['finished','Konec',fin.length],['scheduled','NÃ¡plÃ¡n.',sch.length]].forEach(function(f){
    ...
    [['all','VÅ¡e'],['singles','Dvouhra'],['doubles','ÄtyÅhra']].forEach(function(f){var on=activeFormat===f[0];h+='<button data-fmt="'+f[0]+'" style="padding:2px 9px;border-radius:7px;border:1px solid '+(on?'#f472b6':'rgba(255,255,255,.08)')+';background:'+(on?'rgba(244,114,182,.15)':'transparent')+';color:'+(on?'#f472b6':'rgba(255,255,255,.3)')+';font-size:9px;font-weight:'+(on?700:400)+';cursor:pointer;">'+f[1]+'</button>';});
    h+='</div>';
    ...
    if(!shown.length){h+='<div style="padding:60px;text-align:center;color:rgba(255,255,255,.2);">Å½Ã¡dnÃ© zÃ¡pasy</div>';}
      ...
      if(activeSort==='time'){
        ...
        shown.forEach(function(m){
          ...
          if(isLive){var cv1T=(m.sets1||[])[ns-1]||'0',cv2T=(m.sets2||[])[ns-1]||'0';var isZeroT=m.game1==='0'&&m.game2==='0'||m.game1===''||m.game2==='';if(ns>1)h+='<div style="width:1px;background:rgba(255,255,255,.2);align-self:stretch;margin:0 1px;flex-shrink:0;"></div>';h+='<div style="text-align:center;min-width:28px;background:rgba(0,200,83,.15);border:1px solid rgba(0,200,83,.3);border-radius:4px;padding:2px 4px;margin-left:1px;"><div style="font-size:14px;line-height:1.3;font-weight:700;color:#ffffff;">'+cv1T+'</div><div style="font-size:14px;line-height:1.3;font-weight:700;color:#ffffff;">'+cv2T+'</div></div>';if(m.game1!==''&&m.game2!==''){h+='<div style="text-align:center;min-width:28px;background:'+(isZeroT?'rgba(255,255,255,.05)':'rgba(0,200,83,.25)')+';border:1px solid '+(isZeroT?'rgba(255,255,255,.1)':'rgba(0,200,83,.5)')+';border-radius:4px;padding:1px 4px;margin-left:2px;"><div style="font-size:12px;line-height:1.3;font-weight:800;color:'+(isZeroT?'rgba(255,255,255,.3)':'#00C853')+';">'+m.game1+'</div><div style="font-size:12px;line-height:1.3;font-weight:800;color:'+(isZeroT?'rgba(255,255,255,.3)':'#00C853')+';">'+m.game2+'</div></div>';}}
          h+='</div>';
...
function buildUI(){
  ...
  sidebar.innerHTML=`
    ...
    <nav id="sb-nav">
      ...
      <div class="nav-item" data-view="matches" id="nav-matches"><span class="nav-icon">ð¾</span> ZÃ¡pasy
      ...
      function nx(i){
        ...
        .then(function(cr){
          ...
          if(cd&&cd.source==='tennisabstract'&&cd.updated&&(Date.now()-new Date(cd.updated))/86400000<7){
            sk++;dn++;if(dn%20===0||dn===tot)prog.innerHTML=dn+'/'+tot+' \u2705'+im+' \u23ed'+sk+' \u274c'+er;
