          ...
          .then(function(html){
            ...
            return fetch('https://raw.githubusercontent.com/Havran001/tennis-scout/main/player_history/'+p.id+'.json?v='+Date.now())
              ...
              .then(function(existing){
                ...
                try{var xhr=new XMLHttpRequest();xhr.open('GET','https://raw.githubusercontent.com/Havran001/tennis-scout/main/player_history/'+p.id+'.json?v='+Date.now(),false);xhr.send();if(xhr.status===200){var ex=JSON.parse(xhr.responseText);if(ex&&ex.matches){var om={};ex.matches.forEach(function(m){if(m.odds_opp>0){var k=m.date+'|'+(m.opponent||'').toLowerCase().split(' ').pop();om[k]={alc:m.odds_alc,opp:m.odds_opp,src:m.odds_src};}});if(Object.keys(om).length>0){ms.forEach(function(m){if(m.odds_opp>0)return;if(!(m.score||'').trim())return;var k=m.date+'|'+(m.opponent||'').toLowerCase().split(' ').pop();if(om[k]){m.odds_alc=om[k].alc;m.odds_opp=om[k].opp;m.odds_src=om[k].src;}});console.log('[OddsMerge] restored for '+p.id);}}}}catch(oxe){console.log('[OddsMerge] xhr err '+oxe.message);}
