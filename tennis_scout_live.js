...
var m2=(h2.matches||[]).find(function(x){
  return x.date===m.date&&(x.opponent||'').toLowerCase().split(' ').some(function(w){
    return p1n.toLowerCase().split(' ').some(function(v){return v.length>2&&w.includes(v);});
  });
});
var dateStr=m.date?(m.date.slice(8,10)+'.'+m.date.slice(5,7)+'.'+m.date.slice(0,4)):'';
var surf=m.surface&&m.surface.toLowerCase();
var surfLbl=surf&&surf.includes('clay')?'Antuka':surf&&surf.includes('grass')?'Tráva':'Tvrdý';
var surfClr=surf&&surf.includes('clay')?'#fb923c':surf&&surf.includes('grass')?'#4ade80':'#60a5fa';
var r1='<tr style="background:'+bg+';border-bottom:0;">'
+'<td style="padding:5px 8px;font-size:11px;color:rgba(255,255,255,.4);">'+dateStr+'</td>'
+'<td style="padding:5px 8px;font-size:11px;color:rgba(255,255,255,.4);max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">'+(m.tournament||'—')+'</td>'
+'<td><span style="font-size:9px;font-weight:700;padding:2px 6px;border-radius:3px;background:rgba(255,255,255,.05);color:'+surfClr+';">'+surfLbl+'</span></td>'
+'<td style="padding:5px 8px;font-size:11px;color:rgba(255,255,255,.4);">'+(m.round||'—')+'</td>'
+'<td style="padding:5px 8px;font-size:12px;font-weight:600;font-family:monospace;color:#e6edf3;">'+(m.score||'—')+ws+'</td>'
+'<td></td><td></td><td></td><td></td><td></td><td></td><td></td>'
+'</tr>';
var r2='<tr style="background:rgba(255,255,255,.018);">'
+'<td></td><td></td><td></td><td></td><td></td>'
+'<td style="padding:3px 6px;font-size:10px;color:rgba(255,255,255,.6);font-style:italic;white-space:nowrap;" data-plink="'+p1n+'">'+p1n.split(' ').pop()+'</td>'
+'<td style="padding:3px 6px;font-size:11px;font-family:monospace;text-align:right;color:rgba(255,255,255,.5);white-space:nowrap;">'+(m.dr||'—')+'</td>'
+'<td style="padding:3px 6px;font-size:11px;font-family:monospace;text-align:right;color:rgba(255,255,255,.5);white-space:nowrap;">'+(m.a_pct||'—')+'</td>'
+'<td style="padding:3px 6px;font-size:11px;font-family:monospace;text-align:right;color:rgba(255,255,255,.5);white-space:nowrap;">'+(m.va_pct||'—')+'</td>'
+'<td style="padding:3px 6px;font-size:11px;font-family:monospace;text-align:right;color:rgba(255,255,255,.5);white-space:nowrap;">'+(m.first_pct||'—')+'</td>'
+'<td style="padding:3px 6px;font-size:11px;font-family:monospace;text-align:right;color:rgba(255,255,255,.5);white-space:nowrap;">'+(m.second_pct||'—')+'</td>'
+'<td style="padding:3px 6px;font-size:11px;font-family:monospace;text-align:right;color:rgba(255,255,255,.5);white-space:nowrap;">'+(m.match_time||'—')+'</td>'
+'</tr>';
var r3='<tr style="background:rgba(0,0,0,.1);border-bottom:2px solid rgba(255,255,255,.04);">'
+'<td></td><td></td><td></td><td></td><td></td>'
+(m2
?'<td style="padding:3px 6px;font-size:10px;color:rgba(255,255,255,.6);font-style:italic;white-space:nowrap;" data-plink="'+p2n+'">'+p2n.split(' ').pop()+'</td>'
 +'<td style="padding:3px 6px;font-size:11px;font-family:monospace;text-align:right;color:rgba(255,255,255,.5);white-space:nowrap;">'+(m2.dr||'—')+'</td>'
 +'<td style="padding:3px 6px;font-size:11px;font-family:monospace;text-align:right;color:rgba(255,255,255,.5);white-space:nowrap;">'+(m2.a_pct||'—')+'</td>'
 +'<td style="padding:3px 6px;font-size:11px;font-family:monospace;text-align:right;color:rgba(255,255,255,.5);white-space:nowrap;">'+(m2.va_pct||'—')+'</td>'
 +'<td style="padding:3px 6px;font-size:11px;font-family:monospace;text-align:right;color:rgba(255,255,255,.5);white-space:nowrap;">'+(m2.first_pct||'—')+'</td>'
 +'<td style="padding:3px 6px;font-size:11px;font-family:monospace;text-align:right;color:rgba(255,255,255,.5);white-space:nowrap;">'+(m2.second_pct||'—')+'</td>'
 +'<td style="padding:3px 6px;font-size:11px;font-family:monospace;text-align:right;color:rgba(255,255,255,.5);white-space:nowrap;">'+(m2.match_time||'—')+'</td>'
:'<td colspan="7"></td>')
+'</tr>';
return r1+r2+r3;
      ...
      wrap.querySelectorAll('[data-plink]').forEach(function(el){var nm=el.dataset.plink;el.style.cursor='pointer';el.addEventListener('click',function(){var sh=document.getElementById('ts-host').shadowRoot;sh.getElementById('nav-players').click();setTimeout(function(){var inp=sh.querySelector('#ps-i');if(!inp)return;inp.value=nm.trim().split(' ').pop();inp.dispatchEvent(new Event('input',{bubbles:true}));setTimeout(function(){var row=Array.from(sh.querySelectorAll('tr.pr')||[]).find(function(r){return(r.dataset.fullname||r.dataset.pname||'').toLowerCase()===nm.toLowerCase();});if(row)row.click();},400);},300);});});
    }
