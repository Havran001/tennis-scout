      ...
      function nx(i){
        ...
        .then(function(cr){
          ...
          .then(function(html){
            ...
            var ms=null;if(_mmx&&_mmx.length>=5){ms=_mmx.map(function(mx){var dt=mx[0]||'',ds=dt.length===8?dt.slice(0,4)+'-'+dt.slice(4,6)+'-'+dt.slice(6,8):dt,oa=parseInt(mx[30])||0,ot=parseInt(mx[32])||0;return {date:ds,tournament:mx[1]||'',surface:mx[2]||'',level:'ta-import',round:mx[8]||'',result:mx[4]==='W'?'W':(mx[4]==='L'?'L':''),opponent:mx[11]||'',score:mx[9]||'',best_of:'',rank:'',opp_rank:mx[12]||'',dr:'',a_pct:'',va_pct:ot>0?(oa/ot*100).toFixed(1):'',df_pct:'',first_in:'',first_pct:'',second_pct:'',bp_saved:'',match_time:mx[10]||'',odds:''};});}
            var _noVa=cd&&cd.matches&&cd.matches.length>0&&!("va_pct" in cd.matches[0]);if(!ms||ms.length<5||(ms.length<=cn&&!_noVa)){sk++;dn++;prog.innerHTML=window._tsProgress=dn+'/'+tot+' \u2705'+im+' \u23ed'+sk+' \u274c'+er+(dn===tot&&window._importFailed&&window._importFailed.length?'<br><span style="color:#f87171;font-size:10px;line-height:1.8">\u274c Chyby:<br>'+window._importFailed.join('<br>')+'</span>':'');setTimeout(function(){nx(i+1);},200);return;}
            ...
            .then(function(pr){if(pr.ok){im++;prog.innerHTML=window._tsProgress=dn+'/'+tot+' ✅'+im+' ⏭'+sk+' ❌'+er+' → '+p.full_name+' ('+ms.length+')';}else{pr.json().then(function(e){console.error('GH PUT failed: '+p.full_name+' ('+p.id+') '+e.message);window._importFailed=window._importFailed||[];window._importFailed.push(p.full_name+' ('+p.id+'): '+e.message);});er++;}dn++;setTimeout(function(){nx(i+1);},800);});
          });
