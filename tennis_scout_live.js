...
function buildH2HTab(sh){
  ...
  function renderH2H(p1,p2,hist1,hist2){
    ...
    if(h2h.length){
      ...
      html+='<style>.h2h-win{color:#FFD700;font-weight:700;cursor:pointer;font-size:11px;}.h2h-pn{padding:3px 6px;font-size:10px;color:rgba(255,255,255,.6);font-style:italic;white-space:nowrap;cursor:pointer;}</style><table style="width:100%;border-collapse:collapse;">';
      html+='<thead><tr>';
...
(function waitForH2H(){
  ...
  var timer=setInterval(function(){
    ...
    function renderH2H(p1,p2,h1,h2){
      ...
      if(h2hM.length){html+='<div style="margin-top:14px;"><div style="font-size:9px;color:rgba(255,255,255,.25);letter-spacing:2px;text-transform:uppercase;margin-bottom:8px;padding-bottom:8px;border-bottom:1px solid rgba(255,255,255,.04);">Vzájemné zápasy ('+h2hM.length+')</div><table style="width:100%;border-collapse:collapse;"><thead><tr>'+['Datum','Turnaj','Povrch','Kolo','Skóre','Hráč','DR','A%','vA%','1st%','2nd%','T'].map(function(c){return '<th style="padding:4px 8px;font-size:8px;color:rgba(255,255,255,.25);letter-spacing:1px;text-align:left;border-bottom:1px solid rgba(255,255,255,.06);">'h2hM.map(function(m,i){var bg=i%2===0?'transparent':'rgba(255,255,255,.012)';var wn=m.result==='W'?p1n:p2n;var ws=m.result&&m.score?' <span class=\'h2h-win\' data-plink=\'' +wn+ '\'>'+wn.split(' ').pop()+'</span>':'';var m2=(h2.matches||[]).find(function(x){return x.date===m.date&&(x.opponent||'').toLowerCase().split(' ').some(function(w){return p1n.toLowerCase().split(' ').some(function(v){return v.length>2&&w.includes(v);});});});var ds=m.date?(m.date.slice(8,10)+'.'+m.date.slice(5,7)+'.'+m.date.slice(0,4)):'';var sl=(m.surface||'').toLowerCase();var slbl=sl.includes('clay')?'Antuka':sl.includes('grass')?'Tráva':'Tvrdý';var sclr=sl.includes('clay')?'#fb923c':sl.includes('grass')?'#4ade80':'#60a5fa';var td='padding:5px 8px;font-size:11px;color:rgba(255,255,255,.4);white-space:nowrap;';var tdb='padding:5px 8px;font-size:12px;font-weight:600;font-family:monospace;color:#e6edf3;';var tn='padding:3px 6px;font-size:10px;color:rgba(255,255,255,.6);font-style:italic;white-space:nowrap;';var ts='padding:3px 6px;font-size:11px;font-family:monospace;text-align:right;color:rgba(255,255,255,.5);white-space:nowrap;';var r1='<tr style="background:'+bg+';border-bottom:0;">'+'<td style="'+td+'">'+ds+'</td>'+'<td style="'+td+'max-width:180px;overflow:hidden;text-overflow:ellipsis;">'+(m.tournament||'—')+'</td>'+'<td><span style="font-size:9px;font-weight:700;padding:2px 6px;border-radius:3px;background:rgba(255,255,255,.05);color:'+sclr+';">'+slbl+'</span></td>'+'<td style="'+td+'">'+(m.round||'—')+'</td>'+'<td style="'+tdb+'">'+(m.score||'—')+ws+'</td>'+'<td></td><td></td><td></td><td></td><td></td><td></td><td></td>'+'</tr>';var r2='<tr style="background:rgba(255,255,255,.018);">'+'<td></td><td></td><td></td><td></td><td></td>'+'<td class="h2h-pn" data-plink=\''+(p1n)+'\'>'+p1n.split(' ').pop()+'</td>'+'<td style="'+ts+'">'+(m.dr||'—')+'</td>'+'<td style="'+ts+'">'+(m.a_pct||'—')+'</td>'+'<td style="'+ts+'">'+(m.va_pct||'—')+'</td>'+'<td style="'+ts+'">'+(m.first_pct||'—')+'</td>'+'<td style="'+ts+'">'+(m.second_pct||'—')+'</td>'+'<td style="'+ts+'">'+(m.match_time||'—')+'</td>'+'</tr>';var r3='<tr style="background:rgba(0,0,0,.1);border-bottom:2px solid rgba(255,255,255,.04);">'+'<td></td><td></td><td></td><td></td><td></td>'+(m2?'<td class="h2h-pn" data-plink=\''+(p2n)+'\'>'+p2n.split(' ').pop()+'</td>' +'<td style="'+ts+'">'+(m2.dr||'—')+'</td>' +'<td style="'+ts+'">'+(m2.a_pct||'—')+'</td>' +'<td style="'+ts+'">'+(m2.va_pct||'—')+'</td>' +'<td style="'+ts+'">'+(m2.first_pct||'—')+'</td>' +'<td style="'+ts+'">'+(m2.second_pct||'—')+'</td>' +'<td style="'+ts+'">'+(m2.match_time||'—')+'</td>':'<td colspan="7"></td>')+'</tr>';return r1+r2+r3;}).join('').result)+';"> '+(m.result||'?')+'</td></tr>';}).join('')+'</tbody></table></div>';}
