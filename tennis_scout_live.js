...
setInterval(function(){var sh=(document.getElementById('ts-host')||{}).shadowRoot;if(!sh)return;var hw=sh.getElementById('h2hw');if(!hw)return;var i1=hw.querySelector('#h2h-p1'),i2=hw.querySelector('#h2h-p2');var p1=i1?i1.value.trim():'',p2=i2?i2.value.trim():'';if(!p1||!p2)return;Array.from(hw.querySelectorAll('#h2h-result tr')).forEach(function(row){var tds=row.querySelectorAll('td');if(tds.length<8)return;var wl=tds[7].textContent.trim();if(wl!=='W'&&wl!=='L')return;if(tds[6].querySelector('.h2h-win'))return;var b=document.createElement('b');b.className='h2h-win';b.textContent=' '+(wl==='W'?p1:p2).split(' ').pop();b.style.cssText='color:#FFD700;font-weight:700;';tds[6].appendChild(b);});},500);
setInterval(function(){var sh=(document.getElementById('ts-host')||{}).shadowRoot;if(!sh)return;var hw=sh.getElementById('h2hw');if(!hw)return;var ths=hw.querySelectorAll('#h2h-result th');if(ths.length<9)return;if(ths[8].style.borderLeft)return;ths[8].style.borderLeft='2px solid rgba(255,255,255,.12)';},500);
})();