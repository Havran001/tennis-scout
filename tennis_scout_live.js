...
'albot':'MDA'};window[_key]={};(window.ATP_PLAYERS||[]).forEach(function(p){var sn=p.name.split(' ').slice(1).join(' ').toLowerCase();if(sn)window[_key][sn]=_ioc2flag(p.country);});Object.keys(_ex).forEach(function(k){if(!window[_key][k])window[_key][k]=_ioc2flag(_ex[k]);});Object.keys(window[_key]).forEach(function(k){if(k.includes(' ')){var hk=k.replace(/ /g,'-');if(!window[_key][hk])window[_key][hk]=window[_key][k];}});}var s=(n||'').split(' ')[0].toLowerCase();return window[_key][s]||window[_key][s.replace(/-/g,' ')]||'';}
