...
function renderMatches(data){
    ...
    [{d:-1,l:'VÄera'},{d:0,l:'Dnes'},{d:1,l:'ZÃ­tra'},{d:2,l:'+2 dny'},{d:3,l:'+3 dny'}].forEach(function(x){
...
var _f=JSON.parse(localStorage.getItem('ts_favs')||'[]');if(_f.length){wrap.querySelectorAll('button').forEach(function(_b){var _m=_b.getAttribute('onclick');if(!_m)return;var _i=_m.indexOf("id='")+4;var _j=_m.indexOf("'",_i);var _id=_m.substring(_i,_j);if(_f.indexOf(_id)>-1){_b.style.color='#FFD700';var _svg=_b.querySelector('svg');if(_svg)_svg.style.fill='currentColor';}});}
  ...
  async function tick(){
    ...
    try{
      ...
      // NaÄti zÃ¡pasy a Chance data paralelnÄ
  ...
  setTimeout(function(){
    try{
      ...
      dataEls.forEach(function(el){
        ...
        if(!row||row===c)return;
        // Projdi vÅ¡echny divy v ÅÃ¡dku a hledej ÄÃ­sla kurzÅ¯
...
// === taToBeSur PATCH - přidává chybějící záznamy ===
(function(){
  var _waitForImp=setInterval(function(){
    if(typeof window.__imp!=='function') return;
    clearInterval(_waitForImp);
    var s=window.__imp.toString();
    var patched=s.replace(
      'taToBeSur[sur]||sur',
      '({"aliassime":"auger","connell":"o","oconnell":"o","zandschulp":"van","jong":"de","agut":"bautista","fokina":"davidovich","minaur":"de","schwaerzler":"schwarzler"})[sur]||sur'
    );
    if(patched!==s){
      try{ window.__imp=new Function('return '+patched)(); console.log('[taToBeSur] patched OK'); }
      catch(e){ console.log('[taToBeSur] patch err: '+e.message); }
    }
  },500);
})();
