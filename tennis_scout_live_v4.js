...
setInterval(function(){var host=document.getElementById('ts-host');if(!host||!host.shadowRoot)return;var hw=host.shadowRoot.getElementById('h2hw');if(!hw)return;var vsWidget=hw.querySelector('#h2h-vs-matches');if(!vsWidget)return;var pair=vsWidget.dataset.pair;if(!pair)return;var i1=hw.querySelector('#h2h-p1');var p1=i1?i1.value.trim():'';if(!p1)return;function findP(q){return(window.ATP_PLAYERS||[]).find(function(p){var fn=(p.full_name||p.name||'').toLowerCase();var ql=q.toLowerCase();return fn===ql||fn.split(' ').pop()===ql.split(' ').pop();});}var pl1=findP(p1);if(!pl1)return;var matches=(window.__vsMatchesCache||{})[pl1.id];if(!matches)return;var p2=pair.split('|')[1];if(!p2)return;var p2Last=p2.split(' ').pop().toLowerCase();var vs=matches.filter(function(m){return(m.opponent||'').toLowerCase().indexOf(p2Last)!==-1;});if(!vs.length)return;var W=vs.filter(function(m){return m.result==='W';}).length;var L=vs.filter(function(m){return m.result==='L';}).length;var N=vs.length;var bigSpans=[];hw.querySelectorAll('span').forEach(function(s){var fs=parseFloat(getComputedStyle(s).fontSize);if(fs>25&&/^\d+$/.test(s.textContent.trim()))bigSpans.push(s);});if(bigSpans.length>=2){if(bigSpans[0].textContent.trim()!==String(W))bigSpans[0].textContent=String(W);if(bigSpans[1].textContent.trim()!==String(L))bigSpans[1].textContent=String(L);}hw.querySelectorAll('div').forEach(function(e){if(e.children.length!==0)return;var t=(e.textContent||'').trim();if(/^\d+\s*zápasů$/.test(t)){var exp=N+' zápasů';if(t!==exp)e.textContent=exp;}});},400);
