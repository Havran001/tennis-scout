...
function renderMatches(data){
  ...
  return _c;
    ...
    h+='</div>';
    // Async render: první skupina ihned, zbytek postupně
    (function(){
      var _MARKER='<!--TSGRP-->';
      var _parts=h.split(_MARKER);
      if(_parts.length<3){wrap.innerHTML=h;_attachFilterObs();return;}
      // Zobraz header + první skupinu okamžitě
      wrap.innerHTML=_parts[0]+_MARKER+_parts[1];
      _attachFilterObs();
      // Zbytek skupin přidávej po 3 najednou
      var _i=2,_total=_parts.length;
      function _next(){
        if(_i>=_total){
          wrap.innerHTML=h;
          _attachFilterObs();
        ...
        var _end=Math.min(_i+3,_total);
        wrap.innerHTML=_parts.slice(0,_end).join(_MARKER);
        _i=_end;
        setTimeout(_next,0);
      }
      setTimeout(_next,0);
    })();
  // _attachFilterObs called above
  _activeFilterKey=window._tsActiveFilter||_activeFilterKey;
  if(_activeFilterKey!=='all')_doApplyFilter();
  // Rank range handler
  var rrEl=wrap.querySelector('#ps-rr');
